...
# instantiates quadtree class
const QuadTree = preload("Quadtree.gd")
var quads = QuadTree.new(self)
...
func regenerate_mesh(planet_data : PlanetData):
	...
	# stores the initial mesh data
	quads.set_total_mesh(arrays)
	quads.set_levels(planet_data.radius)
	quads.set_normal(normal)
	quads.subdivide(planet_data.radius)
...
# Uses global player_position variable
func _process(delta):
	# only updates vertices if game is running
	if Global.game_running == true:
		quads.player_nearby(Global.player_position)'''
